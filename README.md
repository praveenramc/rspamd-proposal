...
At the end of this project, a module capable of automatically learning from the spam and ham corpus and re-scoring the weights, nightly will be made available for Rspamd.
...
* #### Automation, Testing & Documentation
    This task will provide a way to configure the automation of previous two tasks, test the module in various environments and document the module.  
...
In our case, each input node represents a symbol. For a particular message, the input vector consists of a series of 0s and 1s (0 if the symbol was not associated with the message, 1 if the symbol was associated with the message). The weight of each input node represents the score of each symbol. A bias value is added to the linear combination of the input and the weights. So, in effect the Perceptron's input will be:
...
* Write scripts to pre-process results and filter out the required data.
...
Apart from these, I have built a Web platform for debugging contests using Django. [Source](https://github.com/cpragadeesh/coldcoffee). Built a command line interface to automate testing for online algorithmic contests conducted in codeforces.com using Python. [Source](https://github.com/cpragadeesh/codeforces-cli).
...
I learnt Lua for modifying a MOD for a game called [MTA SA](https://www.mtasa.com) in my highschool. Although I have been a little out of touch, I could pick up Lua easily since I have used other scripting languages like JS, Python, etc.
